...
import { get, useGetThemeProps, styled } from "style";
import { round } from "utils/helpers";
import { handleKeyboardEvents, keys } from "utils/keys";
import { Color } from "utils/color";
import { useColorMode, useColorModes, useSetColorMode } from "state/editor";
  ...
  ({ onChange, value: controlledValue, variant, themeKey, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        <Swatch {...getThemeProps("swatch")} />
        <InputControls color={color} getThemeProps={getThemeProps} />
      </Container>
    );
  }
);
...
function InputControls({ color, getThemeProps }) {
  const mode = useColorMode();
