...
export function useComboboxProps({
  ...
  const textboxId = id;
  const fallbackTextboxId = seed("TEXTBOX");

  const listboxRef = useRef();
  const listboxId = seed("LIST");
  ...
  function getTextboxProps(props) {
    ...
    return {
      ...rest,
      "aria-activedescendant": optionIds[selectedIndex] || undefined,
  ...
  function getOptionProps(props) {
    ...
    return {
      ...
      "data-focused": option === focusedItem || undefined,
      id: optionIds[index],
      onClick: handleClick,
